...
import { backendApi, SreniContactRowApi } from '../utils/backendApi';
  ...
  const load = useCallback((p: number) => {
    ...
    backendApi.listSreniContacts(sreniId, p, pageSize)
      .then((res) => {
        ...
        if (colSet.size > 0) {
          const masterOrdered = MASTER_CONTACT_COLUMNS
            .map((column) => column.key)
            .filter((key) => colSet.has(key));
          const extras = Array.from(colSet)
            .filter((key) => !MASTER_CONTACT_COLUMN_LABELS.has(key))
    ...
    <div className="animate-slide-up">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary-dark)', fontSize: '0.875rem', marginTop: '4px' }}>
            Contact list uploaded from Excel.{sourceFile && (
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <a
            href="/templates/master-sreni-contact-template.xlsx"
            download
            className="btn btn-secondary"
            style={{ fontSize: '0.875rem', textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}
          >
            Download Template
          </a>
          {total > 0 && (
      ...
      {!isLoading && total === 0 && (
        <div className="glass-panel" style={{ padding: '60px 24px', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-secondary-dark)', fontSize: '0.875rem', marginBottom: '24px', maxWidth: '400px', margin: '0 auto 24px' }}>
            Upload the master contact Excel template (.xlsx or .xls) to populate this contact list.
            The upload validates and maps columns to the standard master-contact table.
          </p>
          <button
            type="button"
            className="btn btn-primary"
                  ...
                  Array.from({ length: 6 }).map((_, i) => (
                    <tr key={i}>
                      ...
                      {Array.from({ length: Math.max(columns.length, 3) }).map((__, j) => (
                        <td key={j}>
                          <div style={{ height: '12px', borderRadius: '4px', background: 'var(--border-dark)', width: `${40 + Math.random() * 40}%`, animation: 'pulse 1.4s ease-in-out infinite' }} />
                        </td>
                      ))}
                    </tr>
                  ))
              ...
              {pageNums.map((n, i) =>
                n === '…' ? (
                  ...
                  <button
                    ...
                    onClick={() => { setPage(n as number); load(n as number); }}
                  >{n}</button>
                )
              )}
              <button
      ...
      <div className="glass-panel" style={{ padding: '14px 18px', marginTop: '20px', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
        ...
        <div style={{ fontSize: '0.82rem', color: 'var(--text-secondary-dark)', lineHeight: 1.6 }}>
          <strong>Master template import:</strong> This table now follows the provided master-contact template columns.
          The upload normalizes template headers (including line-break header cells) and stores all Sreni rows in one unified database table.
          Uploading a new file replaces the existing contact list for this Sreni.
        </div>
      </div>
    </div>
  );
